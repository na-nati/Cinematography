...
import {
  ...
  FaSoundcloud,
  FaCameraRetro,
  FaRegLightbulb, // Using FaRegLightbulb for Lighting Design
  FaApple,
} from 'react-icons/fa';
import {
  SiAdobepremierepro,
...
// Structured skill data with categories
const skillCategories = [
  ...
  },
  {
    ...
    skills: [
      ...
      { name: "Lighting Design", icon: FaRegLightbulb }, // Renamed for clarity
    ]
  },
  {
...
// Animation variant for individual skill icons (drop-in effect)
const rainVariant = {
  ...
  visible: (i) => ({
    ...
    y: 0,
    transition: {
      delay: i * 0.1, // Adjusted delay for a quicker staggered effect
      duration: 0.8,
      ease: 'easeOut'
    }
  })
...
// Component for a single skill icon with hover label
const SkillSymbol = ({ Icon, label, index }) => (
  <motion.div
    custom={index}
    initial="hidden"
    whileInView="visible"
    viewport={{ once: true }} // Ensures animation plays only once when element comes into view
    variants={rainVariant}
    className="relative group p-8 bg-purple-900/20 border border-purple-600 rounded-xl flex justify-center items-center hover:scale-110 transition-all duration-300" // Increased padding
  >
    <Icon className="text-purple-300 text-5xl md:text-6xl" /> {/* Increased icon size */}
...
const Skills = () => {
  // State to track mouse/touch position for parallax effect
  ...
  useEffect(() => {
    ...
    const handleMouseLeave = () => {
      // Reset mouse position to center when mouse leaves window
      setMousePosition({ x: window.innerWidth / 2, y: window.innerHeight / 2 });
    };
    ...
    const handleTouchEnd = () => {
      // Reset touch position to center when touch ends
      setMousePosition({ x: window.innerWidth / 2, y: window.innerHeight / 2 });
    };
    ...
    window.addEventListener('touchcancel', handleTouchEnd); // Also handle touchcancel
  ...
  // Function to calculate parallax style based on mouse/touch position
  ...
  // Restructured to ensure each text appears only once, with unique positions
  const parallaxTexts = [
    ...
    <div className="min-h-screen bg-gradient-to-b from-black via-gray-900 to-black px-4 sm:px-6 py-16 text-white overflow-hidden font-Mightail relative">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none opacity-[0.07] font-Mightail">
        {parallaxTexts.map((item, i) => (
            <div
              key={`${item.text}-${i}`} // Ensure unique key for each rendered div
              className={`absolute ${item.position} text-lg sm:text-xl md:text-3xl lg:text-4xl text-purple-500 transform rotate-${Math.floor(Math.random() * 360)}`} /* Adjusted text sizes for better readability */
      ...
      <motion.h1
        className="text-4xl md:text-5xl font-bold text-center text-purple-400 mb-12 relative z-10" /* Added relative z-10 to ensure title is above parallax */
      ...
      <div className="max-w-6xl mx-auto space-y-12 relative z-10"> {/* Added space-y for gaps between categories, relative z-10 for content */}
        {skillCategories.map((categoryData, catIndex) => (
          <div key={catIndex}>
            {/* Category Title with animation */}
            <motion.h2
              className="text-3xl md:text-4xl font-bold text-center text-gray-200 mb-8 mt-10" // Margin top for separation
            ...
            {/* Grid for skills within the current category */}
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6"> {/* Responsive grid */}
